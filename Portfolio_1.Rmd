...
```{r}
politeness <- read.csv('data/politeness.csv') ## read in data
```
...
Should not be an integer that it can do math with, it is a factor and 1 might as well be called "Talking to teacher"
...
- Looking at subject F5 as an example, the scenario seem to have a bigger impact on voice than attitude.
- For the hypotheses that Koreans lower their voices in formal conditions to hold, the red dots should be above the blue, which is hard to conclude from just the graphs, but might be true.
...
ggplot(politeness, aes(sample=f0mn)) +
  ...
  theme_minimal()
```
We wondered if the data was normal enough to model...? We used the hist() and plotted it to visualize it. 
...
```{r}
# HOw to calculate the residual standard deviation of each model 
sigma(model_2_1)
...
AIC_comparison
```
From the tables we created, we can see that our model_2_4 as the lowest residual standard deviation and AIC.    
...
```
When looking at the R^2, we see again that model_2_4 has the highest value and exlains the most variance.  
...
sigma(model_3_2)
#AIC
AIC1 <- AIC(logLik(model_2_4))
AIC2 <- AIC(logLik(model_3_1))
AIC3 <- AIC(logLik(model_3_2))
AIC1
AIC2
AIC3
```
